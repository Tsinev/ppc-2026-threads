...
InputData GetInputData(const InType &input_tuple) {
  return {std::get<0>(input_tuple), std::get<1>(input_tuple), std::get<2>(input_tuple)};
}
...
void TimurACannonMatrixMultiplication::MultiplyingBlocks(std::vector<std::vector<double>> &result_block,
...
void TimurACannonMatrixMultiplication::ShiftBlocksMatrixALeft(
    std::vector<std::vector<std::vector<std::vector<double>>>> &matrix_blocks_a, int num_block_columns) {
...
void TimurACannonMatrixMultiplication::ShiftBlocksMatrixBUp(
    std::vector<std::vector<std::vector<std::vector<double>>>> &matrix_blocks_b, int num_block_rows) {
...
void TimurACannonMatrixMultiplication::AlgorithmCannon(
    ...
    std::vector<std::vector<std::vector<std::vector<double>>>> &matrix_blocks_c, int block_dimension,
    int num_block_dim) {
...
void TimurACannonMatrixMultiplication::FillingResultingMatrix(
    ...
    std::vector<std::vector<double>> &result_matrix, int block_dimension, int num_block_dim) {
  for (int i = 0; i < num_block_dim; ++i) {
    for (int j = 0; j < num_block_dim; ++j) {
      for (int row = 0; row < block_dimension; ++row) {
        for (int col = 0; col < block_dimension; ++col) {
          result_matrix[(i * block_dimension) + row][(j * block_dimension) + col] = matrix_blocks_c[i][j][row][col];
        }
      }
    }
  }
...
bool TimurACannonMatrixMultiplication::RunImpl() {
  ...
  std::vector<std::vector<std::vector<std::vector<double>>>> matrix_blocks_a(
      num_block_dim, std::vector<std::vector<std::vector<double>>>(
